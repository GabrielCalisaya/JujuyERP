    ...
    <div class="p-8 max-w-7xl mx-auto">
      ...
      <div class="flex flex-wrap items-center gap-2.5 mb-5">
        ...
        <button (click)="soloStockCritico.set(!soloStockCritico())"
          class="flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-[13px] font-medium
        ...
        <select [ngModel]="ordenar()" (ngModelChange)="ordenar.set($event)"
          class="px-3.5 py-2.5 rounded-xl text-[13px] text-neutral-400
      ...
      @if (errorMsg()) {
        <div class="px-4 py-3 rounded-xl bg-red-500/[0.07] border border-red-500/20 mb-4">
          <p class="text-[13px] text-red-400">{{ errorMsg() }}</p>
        </div>
      }
      @if (successMsg()) {
        <div class="px-4 py-3 rounded-xl bg-emerald-500/[0.07] border border-emerald-500/20 mb-4">
          <p class="text-[13px] text-emerald-400">{{ successMsg() }}</p>
        </div>
      }
            ...
            <div class="space-y-0.5">
              @for (p of productosFiltrados(); track p.id) {
                <div class="group flex items-center gap-4 px-4 py-3.5 rounded-xl
                            ...
                            bg-[#121829]/0 hover:bg-white/[0.02] hover:border-l-indigo-500
...
export class ProductosComponent implements OnInit {
  ...
  guardarProducto(): void {
    ...
    req$.subscribe({
      ...
      error: err => { this.errorGuardar.set(err?.error?.detail ?? 'Error al guardar.'); this.guardando.set(false); }
    });
  }
  ...
  eliminarProducto(p: Producto): void {
    ...
    this.http.delete(`${this.API}/productos/${p.id}`).subscribe({
      ...
      error: () => this.errorMsg.set('No se pudo eliminar.')
    });
  }
  ...
  aplicarAumentoMasivo(): void {
    ...
    this.http.put<{ productosActualizados: number }>(
      ...
      error: () => { this.errorMsg.set('No se pudo aplicar el aumento.'); this.actualizando.set(false); }
    });
  }
